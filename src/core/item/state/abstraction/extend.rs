...
use quote::ToTokens;
use syn::{
    FnArg, ItemTrait, PatType, Receiver, ReturnType, Signature, TraitItem, TraitItemMethod,
...
impl<'a>
    From<(
        (&'a ItemTrait, &'a Vec<TypeParam>, &'a Vec<TraitItem>),
        Rc<ModulePath>,
    )> for Trait<'a>
{
    fn from(
        ((item, ty_params, trait_item), path): (
            (&'a ItemTrait, &'a Vec<TypeParam>, &'a Vec<TraitItem>),
            Rc<ModulePath>,
        ),
    ) -> Trait<'a> {
                                    ...
                                    match input {
                                        ...
                                        FnArg::Receiver(Receiver {
                                            ...
                                            let (r1, r2) = match reference {
                                                Some((and, lifetime)) => (
                                                    Some(and.to_token_stream().to_string()),
                                                    Some(lifetime.to_token_stream().to_string()),
                                                ),
                                                None => (None, None),
                                            };
                                            let mutability =
                                                Some(mutability.to_token_stream().to_string());
        ...
        write!(
            ...
            items = escape_html(
                self.items
                    .iter()
                    .map(
                        |&(ref name, ref ty, ref ret): &(String, Vec<String>, String)| format!(
                            "{name}({ty}) -> {ret}",
                            name = name,
                            ty = ty.join(", "),
                            ret = ret
