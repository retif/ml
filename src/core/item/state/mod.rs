        ...
        ItemState {
            ...
            method: properties
                ...
                .filter_map(|(item, path)| match item {
                    Item::Impl(ItemImpl { trait_, items, .. }) if trait_.is_none() => {
            ...
            .and_then(|((item, path), properties)| {
                match item {
                    ...
                    Item::Trait(item) => {
                        ...
                        let ty_params =
                            Box::leak(Box::new(generics.type_params().cloned().collect())); // FIXME
                        let kind: (_, &'a Vec<TypeParam>, &'a Vec<TraitItem>) =
                            (item, ty_params, items);
                        let kind: (Abstract, Vec<&'a (Item, Rc<ModulePath>)>) = (
                            Abstract::Trait((kind, Rc::clone(path)).into()),
                            properties.to_vec(),
                        );
                    ...
                    Item::Struct(item) => {
                        ...
                        let kind = (
                            Abstract::Struct((kind, Rc::clone(path)).into()),
                            properties.to_vec(),
                        );
                        Some(ItemState::from(kind))
                    }
                    ...
                    Item::Enum(item) => {
                        ...
                        let kind: (&'a ItemEnum, &'a Vec<TypeParam>, &'a Vec<Variant>) =
                            (item, ty_params, variants);
                        let kind: (Abstract, Vec<&'a (Item, Rc<ModulePath>)>) = (
                            Abstract::Enum((kind, Rc::clone(path)).into()),
                            properties.to_vec(),
                        );
                        Some(ItemState::from(kind))
                    }
