...
pub struct Method<'a> {
    ...
    func: Vec<(&'a Visibility, String, &'a Punctuated<FnArg, Comma>, Option<&'a Box<Type>>)>,
    path: Rc<ModulePath>,
}
...
impl<'a> From<(&'a Vec<ImplItem>, Rc<ModulePath>)> for Method<'a> {
    fn from((impl_item, path): (&'a Vec<ImplItem>, Rc<ModulePath>)) -> Method<'a> {
        Method::from((impl_item.iter()
                          .filter_map(|item| {
                              if let ImplItem::Method(ImplItemMethod { ref vis, ref sig, .. }) = item {
                                  if let Signature { ident, inputs, output: ReturnType::Default, .. } = sig {
                                      Some((vis, ident.to_string(), inputs, None))
                                  } else if let Signature { ident, inputs, output: ReturnType::Type(_, ref output), .. } = sig {
                   ...
                   .map(|&(vis, ref name, ref inputs, ty)| {
                       match (vis, ty) {
                           (Visibility::Public(_), Some(ty)) => {
                               format!("+{}{}({}) -> {}", DEFAULT_FUNC, name, inputs.to_token_stream().to_string(), ty.to_token_stream().to_string())
                           }
                           (Visibility::Public(_), None) => {
                               format!("+{}{}({})", DEFAULT_FUNC, name, inputs.to_token_stream().to_string())
                           }
                           (_, Some(ty)) => {
                               format!("-{}{}({}) -> {}", DEFAULT_FUNC, name, inputs.to_token_stream().to_string(), ty.to_token_stream().to_string())
                           }
