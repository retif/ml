...
use syn::{
    AngleBracketedGenericArguments, FnArg, GenericArgument, ImplItem, ImplItemMethod,
    PathArguments, PathSegment, ReturnType, Signature,
};
...
impl Implem {
    ...
    pub fn is_association(&self, ty_name: &str) -> bool {
        self.method.iter().any(|(_, _, result)| {
...
impl<'a> From<(&'a Punctuated<PathSegment, Colon2>, &'a Vec<ImplItem>)> for Implem {
    fn from(
        (segments, impl_item): (&'a Punctuated<PathSegment, Colon2>, &'a Vec<ImplItem>),
    ) -> Implem {
        Implem::from((
            segments
                .iter()
